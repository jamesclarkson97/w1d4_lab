...
# daph has 90 fred 30
  def test_loan_money
    ...
    result_daphne = @person5[:monies]
    result_fred = @person4[:monies]
    assert_equal(90, result_daphne)
    assert_equal(30, result_fred)
  end
  ...
  def test_group_snacks
    result = group_snacks(@people).length
  ...
  def test_find_people_with_no_friends
    result = find_people_with_no_friends(@people).length
